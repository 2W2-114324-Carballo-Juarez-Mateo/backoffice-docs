...
- **Propagación híbrida (Kafka + caché):** cambio + `OutboxMessage` en la misma transacción → `GlobalConfigurationChanged` por el topic `administration.events`; cada consumidor (Temas 03/05/08/10) mantiene **caché local con TTL 10 min** (versión + valor) que el evento invalida antes; respaldo ante caída del Backoffice (sirve el último valor conocido).
